  ...
  void getUser() async {
    ...
    if (response.error == null) {
      setState(() {
        ...
        _tempatlahir.text = user!.tempat_lahir ?? '';
        _name.text = user!.name ?? '';
        _agama.text = user!.agama ?? '';
        _alamatlengkap.text = user!.alamat_lengkap ?? '';
        _skill.text = user!.skill ?? '';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          leading: IconButton(
            icon:
                Icon(Icons.arrow_back, color: Color.fromARGB(255, 0, 123, 245)),
            onPressed: () {
